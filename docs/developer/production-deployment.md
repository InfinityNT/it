...
- **docker compose.production.yml** - Production environment (consolidated)

### Production Deployment Options

**Option 1: Standalone with built-in Traefik** (recommended for new deployments)
```bash
docker compose -f docker compose.production.yml --profile with-traefik up -d
```

**Option 2: Using existing external Traefik**
```bash
# Comment out the traefik service in docker compose.production.yml first
# Then run:
...
- For Option 2: Traefik already running (as shown in your main docker compose.yml)
- Domain name pointing to your server (e.g., `it.emurh.com`)
